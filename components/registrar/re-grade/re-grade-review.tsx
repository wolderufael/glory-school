...
const mockReGradeRequests: RegradeAssesmentResponse[] = [
  {
    id: 1,
    regradeStatus: "APPROVAL_REQUESTED",
    regradeReason:
...
type ReGradeStatus = "all" | "APPROVAL_REQUESTED" | "APPROVED" | "REJECTED";
...
export default function ReGradeReview() {
  ...
  const getStatusCounts = () => {
    return {
      ...
      APPROVAL_REQUESTED: mockReGradeRequests.filter(
        (r) => r.regradeStatus === "APPROVAL_REQUESTED"
      ).length,
      APPROVED: mockReGradeRequests.filter(
        (r) => r.regradeStatus === "APPROVED"
      ).length,
      REJECTED: mockReGradeRequests.filter(
        (r) => r.regradeStatus === "REJECTED"
      ).length,
    };
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button
                variant={
                  filter === "APPROVAL_REQUESTED" ? "default" : "outline"
                }
                size="sm"
                onClick={() => setFilter("APPROVAL_REQUESTED")}
                className="text-xs bg-orange-600 hover:bg-orange-700"
              >
                <Clock className="h-3 w-3 mr-1" />
                Pending ({statusCounts.APPROVAL_REQUESTED})
              </Button>
              <Button
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {request.regradeStatus === "REJECTED" &&
                            request.regradeReason && (
